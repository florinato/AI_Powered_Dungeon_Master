...
"""
Main game loop - Orchestrates the complete game flow.
"""
...
from game.utils import game_setup, image_generator, map_generator
...
def run_game():
    ...
    while player_state is None:
        ...
        try:
            ...
            if 1 <= choice_num <= len(available_saves):
                ...
                player_state = game_setup.create_new_character(
                    chosen_world_id, player_name
                )
                presenter.speak(
    ...
    while running:
        ...
        if action_type == "help":
            ...
            description = loc_data.get("generated_description") or loc_data.get(
                "description"
            )
            ...
            saved_image_path = image_generator.generate_image_with_gradio(
                ...
                world_theme,
                historical_context,
                inhabitant_description,
            )
            if saved_image_path:
                presenter.speak(
                    f"Image for {location_name} is ready at: {saved_image_path}"
                )
                ...
                if Image:
                    ...
                    print("PIL not available - cannot display image automatically")
            else:
            ...
            if os.path.exists(blueprint_file):
                ...
                map_path = map_generator.generate_map_image(
                    world_definition, game_state, blueprint_file
                )
                if map_path and os.path.exists(map_path):
                    ...
                    if Image:
                        try:
                            ...
                            print(f"Could not open map image automatically: {e}")
                    else:
                        print("PIL not available - cannot display map automatically")
                else:
        ...
        # Acciones que llaman al game_engine
            ...
            game_state, result = move_player(
                game_state, world_definition, action["direction"]
            )
        elif action_type == "back":
            game_state, result = move_back(game_state, world_definition)
        elif action_type == "pick":
            game_state, result = pick_up_item(
                game_state, world_definition, action["item_name"]
            )
        elif action_type == "drop":
            game_state, result = drop_item(
                game_state, world_definition, action["item_name"]
            )
        elif action_type == "use":
            game_state, result = use_item(
                game_state, world_definition, action["item_name"]
            )
        elif action_type == "unlock":
            game_state, result = unlock_path(
                game_state, world_definition, action["direction"]
            )
        ...
        # Acciones que llaman a los event_handlers
            ...
            game_state = handle_dialogue(
                game_state, world_definition, presenter, ai_provider
            )
        ...
        game_state, quest_progress_result = check_quest_progress(
            game_state, world_definition
        )
        ...
        if result:
            ...
            if result.get("type") == "move":
                ...
                game_state, move_quest_result = check_quest_progress(
                    game_state, world_definition
                )
