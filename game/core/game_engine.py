...
# --- LÃ³gica de Habilidades y Eventos ---
...
def trigger_trap(game_state: dict, trap_info: tuple) -> tuple[dict, dict]:
    ...
    game_state["locations"][game_state["player"]["location"]]["traps"][trap_name][
        "triggered"
    ] = True
    ...
    result = {
        "message": message,
        "type": "trap_triggered",
        "game_over": game_state["player"]["hp"] <= 0,
    }
...
def move_player(
    game_state: dict, world_definition: dict, direction: str
) -> tuple[dict, dict]:
    ...
    if loc_data.get("locked_paths", {}).get(direction, False):
        return game_state, {
            "message": f"The path to the {direction} is locked.",
            "type": "info",
        }
    ...
    active_trap = next(
        (
            (n, d)
            for n, d in new_loc_data.get("traps", {}).items()
...
def move_back(game_state: dict, world_definition: dict) -> tuple[dict, dict]:
    ...
    prev_loc_name = (
        game_state["locations"].get(prev_loc_id, {}).get("name", prev_loc_id)
    )
...
def unlock_path(
    ...
    if success:
        ...
        game_state["locations"][game_state["player"]["location"]]["locked_paths"][
            direction
        ] = False
...
def pick_up_item(
    game_state: dict, world_definition: dict, item_name_input: str
) -> tuple[dict, dict]:
    loc_id = game_state["player"]["location"]
    ...
    if not target_item_with_loc_id:
        return game_state, {
            "message": f"There is no '{item_name_input.replace('_', ' ')}' here.",
            "type": "error",
        }
    ...
    result = {
        "message": f"You picked up {item_for_inventory.get('name', 'an item')}.",
        "type": "inventory_add",
    }

    # Se comprueba el progreso de la misiÃ³n al final del turno.
    return game_state, result
...
def drop_item(
    ...
    if not item_to_drop:
        return game_state, {
            "message": f"You don't have '{item_name_input.replace('_', ' ')}'.",
            "type": "error",
        }
    ...
    loc_id = game_state["player"]["location"]
    item_id_on_ground = item_to_drop.get(
        "id", item_to_drop.get("name", "dropped_item").lower().replace(" ", "_")
    )

    game_state["locations"][loc_id].setdefault("items", {})[
        item_id_on_ground
    ] = item_to_drop

    return game_state, {
        "message": f"You dropped the {item_to_drop.get('name', 'an item')}.",
...
def use_item(
    ...
    if item_type == "healing":
        ...
        healed = min(
            item_to_use.get("healing_amount", 0), player["max_hp"] - player["hp"]
        )
        player["hp"] += healed
        ...
        if "consumable" in item_to_use.get("properties", []):
            ...
            message = (
                f"You used the {item_to_use.get('name')} and restored {healed} HP."
            )
        ...
        return game_state, {
            "message": f"You can't 'use' the {item_to_use.get('name')} like that. Try fighting with it instead.",
            "type": "info",
        }

    return game_state, {
        "message": f"The {item_to_use.get('name')} has no immediate use.",
        "type": "info",
    }
...
def process_combat_turn(game_state: dict, npc_name: str) -> tuple[dict, list[dict]]:
    ...
    if npc["hp"] <= 0:
        ...
        player["xp"] += xp
        turn_results.append(
            {"actor": "System", "action": "defeat", "target": npc_name, "xp": xp}
        )
        ...
        if player["xp"] >= player["xp_to_next_level"]:
            ...
            player["attack"] += 2
            turn_results.append(
                {"actor": "System", "action": "level_up", "new_level": player["level"]}
            )

        # La comprobaciÃ³n de quests se hace al final del turno en el bucle principal.
    ...
    if player["hp"] <= 0:
        turn_results.append({"actor": "System", "action": "player_defeat"})
    return game_state, turn_results
...
def accept_quest(
    game_state: dict, world_definition: dict, quest_id: str
) -> tuple[dict, dict]:
    """AÃ±ade una misiÃ³n a la lista de misiones activas del jugador."""
    player = game_state["player"]
    ...
    if not quest_def:
        return game_state, {
            "message": f"Error: Quest '{quest_id}' not found in world definition.",
            "type": "error",
        }
    ...
    # Se crea el estado inicial de la misiÃ³n para el jugador
    player["active_quests"][quest_id] = {
        "quest_id": quest_id,
        "title": quest_def.get("title", "Untitled Quest"),
        # --- Â¡CORRECCIÃ“N CLAVE AQUÃ! ---
...
def check_quest_progress(
    game_state: dict, quests_definition: dict
) -> tuple[dict, dict | None]:
    """
    Comprueba todas las misiones activas del jugador para ver si se ha cumplido el objetivo.
    Ahora tambiÃ©n maneja la bandera step_completed_flag del Modo Aventura.
    
    Args:
        game_state: El estado completo del juego
        quests_definition: Solo el diccionario de misiones (world_definition['quests'])
    """
    player = game_state["player"]
    ...
    # Obtenemos el estado actual del mundo para las comprobaciones (solo para Modo ExploraciÃ³n)
    defeated_npcs_ids = {
        npc_id
        for loc in game_state.get("locations", {}).values()
    ...
    for quest_id, quest_status in list(
        active_quests.items()
    ):  # Usamos list() para poder modificar el dict mientras iteramos
        if (
            quest_status.get("completed")
            or quest_status.get("status") == "ready_to_turn_in"
        ):
            continue

        quest_def = quests_definition.get(quest_id)
        if not quest_def:
            continue
        ...
        # Encontrar el step actual y su Ã­ndice
        ...
        # --- NUEVA LÃ“GICA: Comprobar bandera de Modo Aventura ---
        step_completed_by_flag = quest_status.get("step_completed_flag", False)

        objective_complete = False

        if step_completed_by_flag:
            # El paso fue completado por handle_quest_scene (Modo Aventura)
            objective_complete = True
            # Resetear la bandera
            quest_status["step_completed_flag"] = False
            print(
                f"[DEBUG] Quest step completed by Adventure Mode flag: {quest_def['title']}"
            )

        elif "objective" in step_def:
            # --- LÃ“GICA DE COMPROBACIÃ“N DE OBJETIVOS TRADICIONAL (Modo ExploraciÃ³n) ---
            objective = step_def["objective"]
            obj_type = objective.get("type")
            obj_id = objective.get("id")  # Este es el ID REAL del item/npc/loc

            print(
                f"[DEBUG] Checking Quest '{quest_def['title']}', Step '{step_def['description']}'"
            )
        ...
        if objective_complete:
            ...
            if is_last_step:
                ...
                quest_status["scene_state"] = {}  # Limpiar el estado de la escena anterior
